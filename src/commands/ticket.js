...
if (sub === 'type-roles') {
  ...
  console.log('[TICKET CREATE] Type: ' + type + ', typeRoles:', typeRoles);
  ...
  const btns = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('ticket:claim:' + id).setLabel('Claim').setStyle(ButtonStyle.Primary),
  ...
  const cfg = await Guild.findOne({ guildId: interaction.guildId });
  if (t.userId !== interaction.user.id && !(await isStaff(interaction.member, cfg))) return interaction.editReply({ embeds: [E.error('No perm', 'Can not close')] });
  t.status = 'closed';
  t.closedAt = new Date();
  ...
  const t = await Ticket.findOne({ channelId: interaction.channelId });
  if (!t) return interaction.editReply({ embeds: [E.error('Not ticket', 'Wrong channel')] });
  if (t.claimedBy !== interaction.user.id && !interaction.member.permissions.has(PermissionFlagsBits.ManageChannels)) return interaction.editReply({ embeds: [E.error('No perm', 'Can not unclaim')] });
  t.claimedBy = null;
  t.status = 'open';
  await t.save();
...
if (!t) return interaction.followUp({ embeds: [E.error(‘Not found’, ‘Ticket not found’)], ephemeral: true });
const cfg = await Guild.findOne({ guildId: interaction.guildId });
if (t.userId !== interaction.user.id && !(await isStaff(interaction.member, cfg))) return interaction.followUp({ embeds: [E.error(‘No perm’, ‘Can not close’)], ephemeral: true });
t.status = ‘closed’;
t.closedAt = new Date();
t.closedBy = interaction.user.id;
await t.save();
...
? cfg.tickets.typeRoles[type]
: (cfg.tickets.supportRoles?.length ? cfg.tickets.supportRoles : cfg.tickets.supportRole ? [cfg.tickets.supportRole] : []);
console.log(’[TICKET SELECT] Type: ’ + type + ‘, typeRoles:’, typeRoles);
cfg.tickets.counter = (cfg.tickets.counter || 0) + 1;
await cfg.save();
const id = ‘ticket-’ + String(cfg.tickets.counter).padStart(4, ‘0’);
const typeInfo = TICKET_TYPES.find(t => t.value === type) || TICKET_TYPES[4];
